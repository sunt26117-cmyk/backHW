...
export function checkMillerRisk(params: {
  ...
  C_gd_pF: number;
  C_iss_pF: number;
  R_g_pulldown_ohm: number;
  L_g_nH: number;
  dv_dt_V_per_ns: number;
  vbus: number;
  hasActiveMillerClamp?: boolean;
}): MillerRiskResult {
  const {
    V_th_min,
    C_gd_pF,
  ...
  const C_gd = C_gd_pF * 1e-12;
  const C_iss = C_iss_pF * 1e-12;
  const C_gs = Math.max(C_iss - C_gd, 1e-12); // avoid div by 0
  const R_g = R_g_pulldown_ohm;
  const L_g = Math.max(L_g_nH * 1e-9, 1e-12);
  const dv_dt = dv_dt_V_per_ns * 1e9;
  
  const T_rise = vbus / dv_dt;
  
  // Im(t) current source
  const getIm = (t: number) => {
    return (t <= T_rise) ? C_gd * dv_dt : 0;
  };
  
  // y[0] = v_gs, y[1] = i_R
  const f0 = (t: number, y0: number, y1: number) => (getIm(t) - y1) / C_gs;
  const f1 = (t: number, y0: number, y1: number) => (y0 - y1 * R_g) / L_g;
  
  // RK4 solver
  let t = 0;
  const dt = 1e-11; // 0.01ns step to ensure stability for LC ringing
  const tEnd = T_rise + 50e-9; // simulate up to 50ns after rise
  let steps = Math.floor(tEnd / dt);
  ...
  for (let i = 0; i < steps; i++) {
    // k1
    const k1_0 = f0(t, y0, y1);
    const k1_1 = f1(t, y0, y1);
    
    // k2
    const k2_0 = f0(t + dt/2, y0 + k1_0 * dt / 2, y1 + k1_1 * dt / 2);
    const k2_1 = f1(t + dt/2, y0 + k1_0 * dt / 2, y1 + k1_1 * dt / 2);
    
    // k3
    const k3_0 = f0(t + dt/2, y0 + k2_0 * dt / 2, y1 + k2_1 * dt / 2);
    const k3_1 = f1(t + dt/2, y0 + k2_0 * dt / 2, y1 + k2_1 * dt / 2);
    
    // k4
    const k4_0 = f0(t + dt, y0 + k3_0 * dt, y1 + k3_1 * dt);
    const k4_1 = f1(t + dt, y0 + k3_0 * dt, y1 + k3_1 * dt);
    
    y0 += (dt / 6) * (k1_0 + 2*k2_0 + 2*k3_0 + k4_0);
  ...
  // Apply active miller clamp if present (approximate attenuation of peak)
  if (hasActiveMillerClamp) {
    vMax *= 0.15;
    vMin *= 0.15;
  }

  // legacy formula for compatibility / reference
  const millerCurrentA = C_gd * dv_dt;
  let vGateInducedV = millerCurrentA * R_g;
  if (hasActiveMillerClamp) vGateInducedV *= 0.15;

  const safetyMarginV = V_th_min - vMax;
  const isRiskOfShootThrough = vMax >= V_th_min;
  ...
  if (isRiskOfShootThrough) {
    ...
    recommendation = `【极度危险：直通炸管隐患】RK4仿真显示米勒振铃峰值 (${vMax.toFixed(2)}V) 已经跨越门极开启阈值 (${V_th_min}V)！考虑共源寄生电感 Ls = ${L_g_nH}nH，高频振铃加剧了直通风险。建议：1. 开启有源米勒钳位；2. 减小Rg或优化PCB降低寄生电感；3. 选择Cgd更小的管子。`;
  } else if (safetyMarginV < 0.6) {
    riskLevel = 'WARNING';
    recommendation = `【临界预警】门极开启安全裕量仅 ${safetyMarginV.toFixed(2)}V (<0.6V)。RK4振铃峰值达 ${vMax.toFixed(2)}V。汽车环境在 125℃~150℃ 时 V_th 会衰减 20%~30%，存在偶发微导通发热风险，建议优化门极关断回路。`;
  }

  return {
    millerCurrentA: Number(millerCurrentA.toFixed(3)),
    vGateInducedV: Number(vGateInducedV.toFixed(2)),
    vGateInducedV_max: Number(vMax.toFixed(2)),
    vGateInducedV_min: Number(vMin.toFixed(2)),
